...
$query = 'SELECT c.id AS cid, c.cat_name, f.id AS fid, f.forum_name, f.disp_position, (select count(id) from '.$db->prefix.'forums where parent_forum_id = f.id) as cpt_subforums FROM '.$db->prefix.'categories AS c INNER JOIN '.$db->prefix.'forums AS f ON c.id=f.cat_id AND f.parent_forum_id = 0 ORDER BY c.disp_position, c.id, f.disp_position;';
$result = $db->query($query) or error('Unable to fetch category/forum list', __FILE__, __LINE__, $db->error());
...
<?php
	if ($cur_forum['cpt_subforums'] > 0){
		$query_sub_forums = 'SELECT id as fid, forum_name, disp_position from '.$db->prefix.'forums where parent_forum_id = '.$cur_forum['fid'];
		$result_sub_forums = $db->query($query_sub_forums) or error('Unable to fetch category/forum list', __FILE__, __LINE__, $db->error());
		while ($cur_sub_forum = $db->fetch_assoc($result_sub_forums))
		{
		?>
								<tr>
									<td class="tcl"><a href="admin_forums.php?edit_forum=<?php echo $cur_sub_forum['fid'] ?>" tabindex="<?php echo $cur_index++ ?>"><?php echo $lang_admin_forums['Edit link'] ?></a> | <a href="admin_forums.php?del_forum=<?php echo $cur_sub_forum['fid'] ?>" tabindex="<?php echo $cur_index++ ?>"><?php echo $lang_admin_forums['Delete link'] ?></a></td>
									<td class="tc2"><input type="text" name="position[<?php echo $cur_sub_forum['fid'] ?>]" size="3" maxlength="3" value="<?php echo $cur_sub_forum['disp_position'] ?>" tabindex="<?php echo $cur_index++ ?>" /></td>
									<td class="tcr"><strong>&nbsp;&nbsp;&nbsp;<?php echo pun_htmlspecialchars($cur_sub_forum['forum_name']) ?></strong></td>
								</tr>
		<?php
		}	
	}
